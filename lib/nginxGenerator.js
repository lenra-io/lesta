...
import { getMergedTranslations, loadTranslations } from '../src/i18n.js';
import { getFilesRecursively, mergeDeep } from '../src/utils.js';
 ...
 */
export default async function nginxGenerator(configuration, builders) {
    ...
    const builderPaths = await Promise.all(
        builders.map(async builder => ({
            builder,
            managedPaths: await builder.getManagedPaths(configuration)
        }))
    );
    const paths = builderPaths.flatMap(({ managedPaths }) => managedPaths);
    ...
    // Build the builders paths
    promises.push.apply(promises,
        builderPaths.flatMap(({ builder, managedPaths }) =>
            managedPaths.map(path =>
                builder.build(configuration, path, options)
                    .then(content =>
 ...
 * @param {import('../src/utils.js').Configuration & {nginx: NginxConfiguration}} configuration The configuration
 * @returns 
 */
function generateNginxConf(configuration) {
    ...
    // Server block
    {
        ...
        // addLine(buffer, tabLevel, `server_name documentation;`);
