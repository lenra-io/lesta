 ...
 */
export async function getFilesRecursively(dir, ignoreHiddenChildren = false) {
  if (!existsSync(dir)) return [];
  const files = await readdir(dir);
  const promises = files
    .filter(f => !ignoreHiddenChildren || !f.startsWith('.'))
    .map(f => join(dir, f))
    .map(p => {
      const stat = statSync(p);
      if (stat.isDirectory())
        return getFilesRecursively(p, ignoreHiddenChildren);
      const ret = [];
      if (stat.isFile())
        ret.push(p);
